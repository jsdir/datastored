...
Model.prototype.set = function(name, value, raw) {
  ...
  this.beforeInput(attributes, function(messages, attributes) {
    if (messages) {
      self.setError(messages);
    } else {
      // Only set registered attributes.
      var data = _.pick(attributes, this.attributeNames);

      // Set changed attributes.
      this.changedAttributes = _.union(this.changedAttributes, _.filter(data,
        function(value, name) {
...
Model.prototype.fetch = function(req, scope, cb) {
  ...
  var attributes = this._requestedAttributes(attributes);
...
Model.prototype._transform = function() {

};

Model.prototype._serialize = function(data) {
  var marshaller = this.factory.marshaller;
...
Model.prototype._onFetch = function(attributes) {
  return attributes;
};
