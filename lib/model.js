 ...
 */
Instance.prototype._getInitialAttributes = function() {
  var self = this;
...
Instance.prototype.getPk = function() {
  return this.attributes[this.model.pkAttribute];
};
...
Instance.prototype._beforeFetch = function(attributes) {
...
Instance.prototype._afterFetch = function(req, data, cb) {
...
Instance.prototype._beforeInput = function(attributes, cb) {
...
Instance.prototype._beforeOutput = function(attributes, cb) {
...
Instance.prototype.toObject = function(scope, raw) {
  if (scope) {
    ...
    var attributeNames = this.model.attributeNames;
  }
  return this.get(attributeNames, raw);
};

Instance.prototype.save = function(req, scope, cb) {
...
Instance.prototype.fetch = function(req, scope, cb) {
  if (!this.isValid) {return cb(this.err);}
  var self = this;
  ...
  this.model._fetchFromDatastores(function(datastore, cb) {
...
Instance.prototype.destroy = function(req, cb) {
  if (!this.isValid) {return cb(this.err);}
  var self = this;
  var pk = this._requirePk();
...
Instance.prototype._serialize = function(data, cb) {
  marshaller.serialize({
    marshaller: this.model.orm.marshaller,
    data: data,
    types: this.options.properties
  }, cb);
};

Instance.prototype._unserialize = function(data, cb) {
...
Instance.prototype._getScope = function(scope) {
  if (!scope) {
...
Instance.prototype._saveToCache = function() {
  /* saves everything to cache and ignore changedAttributes. Does not traverse into relations.*/
};

Instance.prototype._filterAttributes = function(func) {
...
Instance.prototype._onInput = function(attributes, raw) {
  if (!raw) {
...
Instance.prototype._onOutput = function(attributes, raw) {
  if (!raw) {
...
Instance.prototype._onSave = function(attributes, cb) {
...
Instance.prototype._setError = function(err) {
  this.isValid = false;
  this.err = err;
};
...
Model.prototype.create = function(attributes, raw, cb) {
...
Model.prototype.get = function(pk, raw) {
 ...
 */
Model.prototype.find = function(query, cb, transform) {
...
Model.prototype._parseOptions = function(options) {
...
Model.prototype._fetchFromDatastores = function(cached, onDatastore, cb) {
...
Model.prototype._saveToDatastores = function(cached, onDatastore, cb) {
...
/*
Model.prototype._addBackrefsToRelated = function(models) {
  var self = this;
...
Model.prototype._addBackref = function(model, relationName) {
