  ...
  return function(orm, attrName) {
    ...
    return {
      ...
      save: function(name, instance, options, cb) {
        ...
        options.savedInstances = (options.savedInstances || []).concat(instanceTag);
        ...
        existing.then(function(existingInstance) {
          return RSVP.all(_.map(self.model._linkHandlers, function(handler) {
            if (!savedInstance && handler.attrName === name) {
              var promises = [];
              if (existingInstance) {
      ...
      initialize: function() {
        ...
        if (destLink) {
          ...
          // Sync joined links.
          _.each([
            {join: destLink.join, targetModel: this, linkAttr: attrName, o: destLink.attrName},
            {join: assocOptions.join, targetModel: model, linkAttr: destLink.attrName, o: attrName}
          ], function(assoc) {
