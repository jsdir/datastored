...
function groupByDatastore(attributes, groups) {
  ...
  if (matchingGroup) {
    var result = {};
    result[matchingGroup.datastore] = matchingGroup.attributes;
    return result;
  }

  // If no matching datastore was found, revert to layering.
  var result = {};
  return _.reduce(groups, function(result, group) {
    var matchedAttributes = _.intersection(attributes, group.attributes);
    if (matchedAttributes.length > 0) {
      result[group.datastore] = matchedAttributes;
...
module.exports = {
  ...
  groupByDatastore: groupByDatastore
};
