...
Instance.prototype._fetchAttributes = function(options, attributes, cb) {
  ...
  var datastoreOptions = {
    id: id,
    idName: this.model.pkProperty,
    table: this.model.options.table,
  ...
  async.parallel(_.object(_.map(['redis', 'cassandra'], function(datastore) {
    ...
    if (err) {return cb(err);}
    var data = _.merge(results.redis || {}, results.cassandra || {});
    // Call back with data.
...
Instance.prototype._saveData = function(options, data, cb) {
  ...
  if (!this.isNew) {
    ...
    if (replaceIndexes.length > 0) {
      // Get the existing value so that their indexes can be located and
      // replaced.
...
Instance.prototype._saveWithDatastoreOptions = function(
  datastoreOptions, options, data, cb
) {
  var self = this;
  var datastores = this.model._getDatastores();
  var attrGroups = this.model.attrGroups;

  // Determine datastores to save attributes to.
  var uncached = _.difference(
    _.keys(data), attrGroups.cache, attrGroups.cacheOnly
  );

  //// Save uncached attributes to cassandra, `cacheOnly` attributes to redis,
  /// and `cached` attributes to both.
  var datastoreAttributes = {
  ...
  async.parallel(_.map(['redis', 'cassandra'], function(datastore) {
    return function(cb) {
      ...
      // Only save to the datastore if it has an attributes.
    ...
    if (!err) {
      // If the instance was saved successfully, change the status.
      self.isNew = false;
...
Instance.prototype._composeCallbacks = function(callbacks, func, target) {
  var self = this;
  ...
  callbacks = _.compact(_.map(callbacks.reverse(), function(name) {
    ...
    return self.model._getCallback(name, target || self);
  }));
