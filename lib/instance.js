  ...
  this._asyncTransform('save', options, data, function(err, options, data) {
    ...
    _.each(data, function(value, name) {
      ...
      if (attribute.indexDatastore) {
        indexes.push({
          indexStore: attribute.indexDatastore,
          attributeName: name,
          value: value
        });
    ...
    async.series([
      ...
      function(cb) {
        ...
        var datastores = _.keys(self.model._datastoreGroups);
        async.each(datastores, function(datastore, cb) {
          var attributes = self.model._datastoreGroups[datastore];
          var datastoreAttributes = _.intersection(attributes, dataKeys);

          if (datastoreAttributes.length > 0) {
            // Only save if the datastore has attributes is common.
            datastore.hashStore.save({
              keyspace: self.model.options.keyspace,
              id: self.getId(true),
...
Instance.prototype._fetch = function(attributes, options, cb) {
  ...
  var groups = utils.groupByDatastore(attributes, this.model._datastoreGroups);
  var datastores = _.keys(groups);
  async.each(datastores, function(datastore, cb) {
    var attributes = groups[datastore];
    datastore.fetch({
      keyspace: self.model.options.keyspace,
      id: self.getId(true),
      attributes: attributes,
