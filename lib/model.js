...
var debug = require('debug')('datastored:model');
...
function mergeProps(mixins) {
  return merge.apply(merge, _.map(mixins, function(mixin) {
...
Model.prototype._filterAttributes = function(option) {
  var attributes = this._attributes;
  return _.filter(this._attributeNames, function(name) {
    return attributes[name][option];
  });
};
...
Model.prototype._initAttributes = function() {
  ...
  _.each(this._attributes, function(attribute) {
    if (attribute.initialize) {
      attribute.initialize.call(self);
    }
  });
};
...
Model.prototype._load = function() {
  ...
  this._counters = this._filterAttributes('counter');
  this._ignoreIOTypes = this._filterAttributes('ignoreIOType');

  this._initAttributes();

  debug('finished _load:', this.type);
};
