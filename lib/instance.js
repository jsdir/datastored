...
var debug = require('debug')('datastored:instance');
...
Instance.prototype.get = function(attributes, options) {
  debug(this.id, 'get:', attributes, options);
  var self = this;
...
Instance.prototype.fetch = function(attributes, options) {
  debug(this.id, 'fetch:', attributes, options);
  var self = this;
  ...
  return RSVP.all(_.map(groups, function(group) {
    ...
    if (attributeGroups.fetch && _.all(results, _.isNull)) {
      return RSVP.resolve(null);
    }

    // Combine starting data (null) for virtual attributes.
    results.push(utils.mapObjectValue(attributeGroups.virtual, null));

    // Combine all of the results.
    var data = _.extend.apply(_.extend, [{}].concat(results));
...
Instance.prototype.save = function(data, options) {
  debug(this.id, 'save:', data, options);
  var self = this;
  var normOptions = options || {};
  ...
  return (_.isNull(this.id) ? self._deferredId.promise : RSVP.resolve())
    ...
    .then(function() {
      ...
      // instance.
      // Counters are incremented after save.
      self._updateCounters(inputData);
      // Since the counters are already updated, do not set them to the
      // increment value.
      _.extend(self._data, _.omit(inputData, self.model._counters));
      self.saved = true;
      return self;
    });
};
...
Instance.prototype._updateCounters = function(values) {
  var self = this;
  var counterData = _.pick(values, this.model._counters);
  _.each(counterData, function(value, name) {
    self._data[name] = self._data[name] || 0;
    self._data[name] += utils.getIncrAmount(value);
  });
};
...
Instance.prototype._persistData = function(data, options) {
  ...
  return RSVP.resolve()
    ...
    .then(function(saveData) {
      return self._saveAttributes(saveData, options.insert);
    })
    .then(function() {
      if (_.isEmpty(data)) {
        return;
      }
      return self.model.orm.events.emit('save', self, data, options);
    });
};
...
Instance.prototype._saveAttributes = function(data, insert) {
  var self = this;
  ...
  return RSVP.all(_.map(this.model._hashStoreGroups, function(group) {
    return new RSVP.Promise(function(resolve, reject) {
      ...
      var storeData = _.pick(data, datastoreAttributes);
      // Divide the incoming data into set and incr data.
      var incrData = _.pick(storeData, self.model._counters);
      var setData = _.omit(storeData, self.model._counters);

      if (datastoreAttributes.length > 0) {
        // Only save if the datastore it has the attributes.
        group.hashStore.save({
          // A list of counters for the hashStore to set to 0 on insert
          counters: self.model._counters,
          // Atomic increments to perform on counters
          incr: incrData,
          keyspace: self.model._props.keyspace,
          id: self.id,
          // Values to be saved directly
          data: setData,
          types: self.model._attributeTypes,
          insert: insert
        }, function(err) {
