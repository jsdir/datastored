...
function Model(name, orm, options) {
  ...
  utils.requireAttributes(this.options, ['column']);
  ...
  // Set attributes.
  this.attributes = _.extend(this.options.properties, this.options.relations);

  var _this = this;
  var groupOptions = ['primary', 'immutable', 'hidden'];
  this.attrGroups = _.object(_.map(groupOptions, function(groupOption) {
    return [groupOption, _.filter(_.keys(_this.attributes), function(name) {
      return _this.attributes[name][groupOption];
    })];
  }));
  ...
  // Check that primary key property is not hidden.
 ...
 */
Instance.prototype.getPkValue = function() {
  return this.values[this.pkProperty];
};
...
Instance.prototype._requirePkValue = function() {
  ...
  if (value) {
    ...
    throw new Error('the model primary key "' + this.pkProperty +
      '" must be set');
  }
};
