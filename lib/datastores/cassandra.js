  ...
  async.series([
    ...
    function(cb) {
      ...
      var query = util.format('UPDATE "%s" SET %s WHERE id = ?', options.table,
        queryValues[0].join(','));
      self._execute(query, queryValues[1].concat([options.id]), {prepare: true}, cb);
    },
    function(cb) {
      // Delete any null values.
      if (nullAttrs.length > 0) {
        var query = util.format('DELETE %s FROM "%s" WHERE id = ?',
          makeNames(_.map(nullAttrs, quote)), options.table);
        self._execute(query, [options.id], {prepare: true}, cb);
      } else {
        cb();
      }
    }
...
CassandraDatastore.prototype.fetch = function(options, cb) {
  ...
  async.map(options.ids, function(id, cb) {
    var query = util.format('SELECT %s FROM "%s" WHERE id IN %s',
      makeNames(_.map(options.attributes, quote)), options.table,
      makeParams(options.ids.length));
    self._execute(query, options.ids, {prepare: true}, function(err, data) {
      if (err) {return cb(err);}
...
CassandraDatastore.prototype.destroy = function(options, cb) {
  var query = util.format('DELETE FROM "%s" WHERE id IN %s', options.table,
    makeParams(options.ids.length));
  this._execute(query, options.ids, {prepare: true}, cb);
};
...
CassandraDatastore.prototype.reset = function(cb) {
  ...
  async.each(this.tables, function(table, cb) {
    ...
    self._execute(query, {prepare: true}, cb);
  }, cb);
};
...
CassandraDatastore.prototype._executeQueries = function(queries) {
  if (queries.length > 1) {
    ...
    this.client.batch(_.map(queries, function(query) {
