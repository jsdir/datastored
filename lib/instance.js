  ...
  if (modelData) {
    if (_.has(modelData, 'id')) {
      ...
      this.id = model._transforms.input
        .call(this, {id: modelData.id}, options).id;
      this._deferredId.resolve(this.id);
    }

    if (modelData.data) {
      this._data = model._transforms.input
        .call(this, this._filterData(modelData.data), options);
    }
  } else {
    // New instance.
    this._generateId();
...
Instance.prototype.save = function(data, options) {
  ...
  var inputData = transforms.input
    .call(this, this._filterData(data), options || {});
  ...
  return new RSVP.Promise(function(resolve, reject) {
    ...
    self._deferredId.promise.then(function(id) {
      transforms.save.call(self, inputData, options, function(err, data) {
        if (err) {return reject(err);}
        resolve(data);
      });
    }).catch(reject);
