...
var AttributeTransforms = {
  input: function(data, applyUserTransforms) {
    ...
    return _.mapValues(data, function(value, name) {
      var attribute = self._getAttribute(name);
      if (attribute.input) {
        value = attribute.input.call(self, name, value, applyUserTransforms);
      }
      return value;
    });
  },
  ...
  outputAsync: function(data, options, applyUserTransforms, cb) {
    ...
    utils.mapValues(data, function(value, name, cb) {
      ...
      if (attribute.outputAsync) {
        attribute.outputAsync.call(self, name, data[name], options,
  ...
  fetch: function(data, cb) {
    ...
    utils.mapValues(data, function(value, name, cb) {
      var attribute = self._getAttribute(name);
      if (attribute.fetch) {
        attribute.fetch.call(self, name, data[name], cb);
      } else {
        cb(null, value);
      }
    }, cb);
  ...
  save: function(data, cb) {
    ...
    utils.mapValues(data, function(value, name, cb) {
      var attribute = self._getAttribute(name);
      if (attribute.save) {
        attribute.save.call(self, name, data[name], cb);
      } else {
        cb(null, value);
      }
    }, cb);
