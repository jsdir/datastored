...
Collection.prototype.fetchAllIds = function(cb) {
...
var HasOne = {
  useProperties: true,
  serializeRelation: function(model, relationName, value) {
    if (value) {
      return value.getPk();
    } else {
      return null;
  ...
  unserializeRelation: function(model, relationName, value) {
    var orm = model.modelConstructor.orm;
    var relation = model.options.relations[relationName];
    return new orm.models[relation.relatedModel].get(value, true);
  },
...
var HasMany = {
  getInitialValue: function(options) {
