 ...
 */
RedisDatastore.prototype.insert = function(attributes, cb) {
...
RedisDatastore.prototype.update = RedisDatastore.prototype.insert;
...
CassandraDatastore.prototype.fetch = function(pkValue, attributes, cb) {
  ...
  var query = 'SELECT ' + _.map(attributes, function(name) {
    ...
    + ' FROM ' + options.table + ' WHERE ' + options.pkAttribute + ' = ?';
...
/**
 * Insert a model in cassandra.
 */
CassandraDatastore.prototype.insert = function(attributes, cb) {
  // Insert the model.
  var self = this;
  var table = this.options.table;
  var pkAttribute = this.options.pkAttribute;
...
/**
 * Update a model in cassandra.
 */
CassandraDatastore.prototype.update = CassandraDatastore.prototype.insert;
/*
CassandraDatastore.prototype.update = function(attributes, cb) {
  // Insert the model.
  var self = this;
  var table = this.options.table;
  var pkAttribute = this.options.pkAttribute;
  ...
  this.getConnection(function(err) {
    if (err) {
      ...
      // Update the model.
      var values = _.values(serializedAttributes);
      var cqlValues = _.flatten(values);
      cqlValues.push(val);
      var placeholder = CassandraDatastore.generateSetPlaceholder(keys);
      self.cassandra.cql(
        'UPDATE ' + table + ' SET ' + placeholder + ' WHERE ' + pkAttribute
          + ' = ?', cqlValues
      , cb);
    }
  });
