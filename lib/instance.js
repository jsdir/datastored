...
Instance.prototype.set = function(name, value, raw) {
  ...
  // Only set values of mutable attributes.
  var self = this;
  var mutableAttributes = _.filter(_.keys(this.model.options.attributes), function(name) {
    var attribute = self.model.options.attributes[name];
    return (attribute.hasValue !== false);
  });

  if (this.isNew) {
    mutableAttributes.push('id');
  }

  values = _.pick(values, mutableAttributes);

  // Transform the attributes if necessary.
  if (!raw) {
    // Remove guarded values.
    var guardedAttributes = _.filter(_.keys(this.model.options.attributes), function(name) {
      var attribute = self.model.options.attributes[name];
      return attribute.guarded;
    });
    values = _.omit(values, guardedAttributes);
    var changedValues = this._transform('input', values);
  } else {
  ...
  if (changedValues) {
    ...
    var changed = _.without(_.keys(changedValues), 'id');
