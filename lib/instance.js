...
Instance.prototype.save = function(data, applyUserTransforms) {
  ...
  return this._deferredId.promise.then(function(id) {
    var inputData = transforms.input.call(self, data, applyUserTransforms);
    transforms.save.call(self, inputData, function(err, transformedData) {
      // mixin defaults if the first save
      if (!self.saved) {
        data = _.extend({}, self.model._getDefaults(), data);
      }

      _.extend(self._data, data);
      self.saved = true;
      // var indexTypes = self._getIndexTypes(transformedData);
    });
    return self;
  });
};
