...
Model.prototype.save = function(req, scope, cb) {
  ...
  // get all attributes that changedProperties includes, just iterate through backrefs like destroy.
...
Model.prototype.fetch = function(req, scope, cb) {
  var pk = this.getPk();
  if (!pk) {
    throw new Error('model must have a defined primary key before fetching');
  }
  ...
  if (cb === null) {
    ...
    if (scope) {
      var baseAttributes = this.getScope(scope);
    } else {
      var baseAttributes = this.relationAndAttributeNames;
    }
  }

  // Filter the attributes by including all properties and all relations that
  // are set to use attributes.
  var fetchAttributes = _.filter(baseAttributes, function(value, name) {
    if (name in this.options.relations) {
      var relation = this.options.relations[name];
      return relation.type.usesAttribute;
    }
    return true;
  });

  var attributes = this.mixins._getAssocNames(fetchAttributes); // adds _permission fields (included model _permission if the model itself has permissions)
  ...
  this.modelConstructor._fetchFromDatastores(function(datastore, cb) {
    ...
    if (data) {
      // TODO: use custom type map for (un)serialization
      _onDatastoreFetch(req, self.transform(data, 'fetch'), function(err, data) {
        if (err) {return cb(err);}
        var filtered = {};
        _.each(data, function(value, name) {
          if (name in self.options.attributes) {
            filtered[name] = value;
          } else if (name in self.options.relations) {
            var relation = self.options.relations[name];
            filtered[name] = relation.createFromValue(value);
          }
        });
...
Model.prototype.destroy = function(req, cb) {
  ...
  // Check if the access request user is authorized to destroy the model.
  if (req.user) {
  ...
  var self = this;
  var relationNames = _.values(this.modelConstructor._backrefs);
  this.fetch(relationNames, function(err) {
    if (err) {return cb(err);}
...
Model.prototype._getScope = function(scope) {
  if (!scope in this.options.scopes) {
...
function ModelConstructor(name, orm, options) {
  ...
  // TODO: check that scopes contain only properties and relations
  // TODO: check that property and relation names do not occlude
}

ModelConstructor.prototype.parseOptions = function(options) {
...
ModelConstructor.prototype.create = function(attributes, cb) {
  ...
  return model.set(attributes || {}, cb);
};
...
ModelConstructor.prototype.deserialize = function(attributes) {
  this.marshaller.deserialize(attributes);
};
