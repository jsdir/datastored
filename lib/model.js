...
Model.prototype.find = function(name, value, raw, cb) {
  ...
  // Create a new instance with the query attribute set.
  var instance = new this.instance(this);
  ...
  datastores.redis.find({
    ...
    if (_.isNull(id)) {
      // The instance was not found. Call back with `null`.
      cb(null, null);
    } else {
...
Model.prototype._initializeAttrGroups = function() {
  ...
  var attrGroups = _.object(_.map(groupOptions, function(all, option) {
  ...
  // Fail if any attributes were defined with both the `cache` and `cacheOnly`
  // properties.
  var invalid = _.intersection(attrGroups.cache, attrGroups.cacheOnly);
  if (invalid.length > 0) {
    throw new Error('attribute "' + invalid[0] + '" cannot have both ' +
      '"cache" and "cacheOnly" defined');
  }
