...
function HasOne(relatedModel, options) {
  return function(orm) { // <- optionally return a wrapper function that will be called with all qualified orm models.
    ...
    // If the relation has multiple types, store an additional attribute
    // that indicates what type the child is.
    var otherAttributes = {};
    var typeAttribute = null;
    if (_.isArray(relatedModel) && relatedModel.length > 1) {
      typeAttribute = name + '_type';
      otherAttributes = {typeAttribute: datastored.fromType(relatedModel)}; // string or integer
    }
    ...
    return { // attributeSpec
      attribute: datastored.fromType(relatedModel.idType)({
        ...
        onSet: function(value) {
          ...
          // If the relation has multiple types, additionally store the type of the child.
          if (typeAttribute) {
            this.set(typeAttribute, instance.type);
          }
