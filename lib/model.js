...
function Model(type, orm, options) {
  ...
  this._datastoreGroups = this._getDatastoreGroups();
...
Model.prototype._getDatastoreGroups = function() {
  var attributes = this.options.attributes;
  return _.reduce(attributes, function(result, attribute, name) {
    _.each(attribute.datastores || [], function(datastore) {
      result[datastore] = result[datastore] || [];
      result[datastore].push(name);
    });
    return result;
  }, {});
};
