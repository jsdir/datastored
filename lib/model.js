...
function Model(modelConstructor) {
  ...
  this.relationNames = _.keys(this.options.relations);

  this.dataNames = _.union(attributeNames, relationNames);
}
...
Model.prototype.set = function(attributes, value, transform) {
  // Set multiple attributes at first.

  if (!_.isObject(attributes)) {
    // Set a single attribute.
    var attributes = _.object([[attributes, value]]);
  } else {
  ...
  attributes = _.pick(attributes, this.dataNames);
  ...
  this.changedAttributes = _.union(this.changedAttributes, _.keys(attributes));
