...
function unserialize(values, cb) {
  cb(null, marshallers.unserialize(
    this.model.orm.marshaller, values, this.model.attributesByType
  ));
}
...
function Model(name, orm, options) {
  ...
  this.attributesByType = _.mapValues(this.attributes, function(options) {
    return options.type;
  });
...
Instance.prototype.save = function(options, cb) {
  ...
  this._composeCallbacks(['beforeSave', function(options, values, cb) {
    ...
    var datastores = self._getDatastores();

    async.parallel([
      function(cb) {
        // Save to redis.
        var datastore = datastores.redis;
        datastore.save(datastoreOptions, cb);
      },
      function(cb) {
        // Save to cassandra.
        var datastore = datastores.cassandra;
        datastore.save(datastoreOptions, cb);
      }
    ], function(err) {
      cb(err, options, values);
    });
    ...
    var datastoreOptions = {
      ...
      data: values,
      types: self.model.attributes,
      indexes: self.model.attrGroups.index,
      // TODO: increments: this.increments
    };
  }, 'afterSave'], async.compose)(options, this.changedAttributes,
    function(err) {
