...
/**
 * Merges model options and attributes.
 */
function mergeOptions(mixins) {
  return _.merge.apply(_.merge, mixins);
}
...
function mergeAttrs() {
  /*
  check if attributes even exists with requireAttributes
   */

  // Load static and deferred attributes.
  var mixins = _.map(initialMixins, function(mixin) {
    var attributes = _.mapValues(mixin.attributes, function(value, name) {
      return [name, _.isFunction(value) ? value(orm) : value];
    });
    return _.extend({}, mixin, {attributes: attributes});
  });
}
...
function Model(type, orm, options) {
  ...
  this.mixins = [{}, options].concat(options.mixins || []);
  this.props = mergeProps(this.mixins);
  utils.requireAttributes(this.props, ['keyspace', 'id']);

  // Set static properties.
  _.extend(this, this.props.statics);

  // Set instance methods.
  this._constructor = _.clone(Instance);
  _.extend(this._constructor.prototype, this.props.methods);
}
...
Model.prototype._load = function() {
  var orm = this.orm;
  this.attributes = mergeAttrs(this.mixins);
