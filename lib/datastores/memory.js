...
MemoryDatastore.prototype.addToCollection = function(options, cb) {
  ...
  var ids = _.map(options.instances, function(instance) {
    var idValue = serialize(instance.id);
  ...
  this.data = update(this.data, {collections: {name: {$push: ids}}});
  cb();
};
...
MemoryDatastore.prototype.fetchCollection = function() {
  var collection = this.data.collections[options.id, options.table];
  if (!collection) {return cb(null, []);}

  var ids = collection.slice(options.start, options.end);
  ...
  cb(null, _.map(ids, function(idValue) {
    ...
    if (options.multiType) {
      ...
      var id = idValue;
      data.id = unserialize(id, options.childIdType);
    }
